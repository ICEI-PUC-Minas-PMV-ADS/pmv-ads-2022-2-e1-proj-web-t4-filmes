class Carousel extends HTMLElement{
    ...
    createCards(){
        ...
        if(this.getAttribute('type') == 'adultAnimation'){
            let cards = adultAnimation.map(mov => {
                if(mov.local_status){
        ...
        else if(this.getAttribute('type') == 'action'){
            let cards = action.map(mov => {
                if(mov.local_status){
                    return this.createCard(`../../assets/imgs/${mov.file_name}`, mov.id)
                }
                else{
        ...
        else if(this.getAttribute('type') == 'terror'){
            let cards = terror.map(mov => {
                if(mov.local_status){
                    return this.createCard(`../../assets/imgs/${mov.file_name}`, mov.id)
                }
                else{
    ...
    defineAtrributes(){
        if(this.getAttribute('type') == null){
            if(Number(sessionStorage.getItem('movie')) <= 9) this.setAttribute('type', 'adultAnimation')
            else if(Number(sessionStorage.getItem('movie')) <= 18) this.setAttribute('type', 'action')
            else if(Number(sessionStorage.getItem('movie')) <= 27) this.setAttribute('type', 'terror')
        }
    }
}
